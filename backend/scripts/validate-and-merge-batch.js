...
function main() {
  const { batchPath, dryRun } = parseArgs();

  if (!fs.existsSync(batchPath)) {
    throw new Error(`Batch introuvable: ${batchPath}`);
  }
  ...
  if (!fs.existsSync(templatePath)) {
    throw new Error(`Fichier introuvable: ${templatePath}`);
  }

  const batch = readJson(batchPath);
  const existingExercises = readJson(exercisesPath);
  const enums = readJson(enumsPath);
  const schema = readJson(schemaPath);
  const template = readJson(templatePath);

  if (!Array.isArray(batch)) {
    throw new Error("Le batch doit être un array JSON.");
  }

  if (!Array.isArray(existingExercises)) {
    throw new Error("exercises.json doit être un array JSON.");
  }

  const allExistingIds = new Set(existingExercises.map((item) => item.exerciseId));
  const allExistingNames = new Set(existingExercises.map((item) => normalizeName(item.name)));
  const batchIdsSeen = new Set();
  const batchNamesSeen = new Set();

  const results = batch.map((exercise, index) => {
    ...
    return {
      index,
      name: exercise && exercise.name ? exercise.name : `index_${index}`,
      exerciseId: exercise && exercise.exerciseId ? exercise.exerciseId : null,
      errors,
    };
  });
  ...
  if (invalid.length > 0) {
    console.error(`Batch invalide. ${invalid.length} exercice(s) ont des erreurs.\n`);

    for (const item of invalid) {
      console.error(`Exercice ${item.index + 1}: ${item.name}`);
      if (item.exerciseId) {
        console.error(`exerciseId: ${item.exerciseId}`);
      }
  ...
  console.log(`Batch valide: ${batch.length} exercice(s).`);

  if (dryRun) {
    console.log("Mode dry-run actif. Aucune fusion effectuée.");
    return;
  }

  const merged = [...existingExercises, ...batch];
  writeJson(exercisesPath, merged);

  console.log(`Fusion terminée dans: ${exercisesPath}`);
}
